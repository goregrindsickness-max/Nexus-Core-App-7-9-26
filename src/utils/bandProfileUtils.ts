...
export const isVirulentExcisionProfile = (activeBand: any, userProfile: any): boolean => {
  ...
  if (bName.includes('virulent') || bName.includes('excision')) return true;
  if (
    userProfile?.name === 'Miguel Goregrinder Medina' || 
...
export const resolveBandName = (activeBand: any, userProfile: any): string => {
  if (activeBand?.name && activeBand.name !== 'Band Workspace') return activeBand.name;
  if (activeBand?.band_name) return activeBand.band_name;
  if (userProfile?.band_name) return userProfile.band_name;
  if (userProfile?.bandName) return userProfile.bandName;
  if (isVirulentExcisionProfile(activeBand, userProfile)) {
