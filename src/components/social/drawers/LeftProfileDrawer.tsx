...
import {
  ...
  Library, Download, Star, ArrowRight, Database, CreditCard, Crown, Wifi, WifiOff, Settings2, LayoutGrid, ShoppingCart, Music, Image as ImageIcon, Shirt, Fingerprint
} from 'lucide-react';
...
export const LeftProfileDrawer: React.FC<LeftProfileDrawerProps> = (props) => {
  ...
  const [loyaltyCustomTiers, setLoyaltyCustomTiers] = React.useState<any[]>([
    { id: 1, name: 'Pit Regular', points: 500, reward: '10% Merch Discount' },
    { id: 2, name: 'Tour Veteran', points: 1500, reward: 'Exclusive Pin + VIP Badge' },
    { id: 3, name: 'Backstage Elite', points: 3500, reward: 'Free Test Pressing LP' }
  ]);
  ...
  const [activeOAuthProvider, setActiveOAuthProvider] = React.useState<'google' | 'apple' | 'paypal' | null>(null);
  const [walletsState, setWalletsState] = React.useState<UserWalletsState>(() => getStoredWallets(props.userProfile));
  ...
  React.useEffect(() => {
    const handleWalletsUpdate = (e: any) => {
      if (e.detail) {
        ...
        setWalletsState(getStoredWallets(props.userProfile));
      }
    };
  ...
  const isGooglePayConnected = Boolean(walletsState?.google?.linked);
  const isApplePayConnected = Boolean(walletsState?.apple?.linked);
  const isPaypalConnected = Boolean(walletsState?.paypal?.linked);

  const handleConnectPayment = (method: 'google' | 'apple' | 'paypal') => {
    // Open OAuth authentication gateway modal
    setActiveOAuthProvider(method);
  };

  const handleDisconnectPayment = (method: 'google' | 'apple' | 'paypal') => {
    const next = disconnectWalletProvider(method, props.userProfile);
    setWalletsState(next);
    const name = method === 'google' ? 'Google Pay' : method === 'apple' ? 'Apple Pay' : 'PayPal Wallet';
    triggerNotification?.(`Unlinked and revoked OAuth access for ${name}.`);
  };

  const handleSelectCard = (provider: 'google' | 'apple' | 'paypal', cardId: string) => {
    const next = selectWalletCard(provider, cardId, props.userProfile);
    setWalletsState(next);
    triggerNotification?.(`Updated primary card for ${provider === 'google' ? 'Google Pay' : provider === 'apple' ? 'Apple Pay' : 'PayPal Wallet'}.`);
  };

  const handleSetPreferredWallet = (method: 'google' | 'apple' | 'paypal') => {
    const next = setPreferredWallet(method, props.userProfile);
    setWalletsState(next);
    const name = method === 'google' ? 'Google Pay' : method === 'apple' ? 'Apple Pay' : 'PayPal Wallet';
    triggerNotification?.(`Set ${name} as default wallet for instant purchases.`);
  };

  const handleToggleSingleTap = () => {
    const next = toggleSingleTapBuying(!walletsState.singleTapEnabled, props.userProfile);
    setWalletsState(next);
    triggerNotification?.(next.singleTapEnabled ? '⚡ Single-Tap buying enabled for all presales & merch.' : 'Single-Tap buying disabled.');
  };
        ...
        <div className="fixed inset-0 z-[250] flex">
          ...
          <div className="relative w-[85%] sm:w-80 max-w-sm bg-[#030303] border-r border-zinc-900 h-full flex flex-col animate-in slide-in-from-left duration-300 shadow-[0_0_80px_rgba(0,0,0,0.95)] overflow-hidden bg-[linear-gradient(to_right,rgba(239,68,68,0.06)_1px,transparent_1px),linear-gradient(to_bottom,rgba(239,68,68,0.06)_1px,transparent_1px)] [background-size:20px_20px]">
            ...
            <div className="flex-1 relative z-10">
                    ...
                    <div className="flex-1 p-4">
                      {drawerCurrentView === 'profile' && (
                        !(portalRole === 'industry_pro' || portalRole === 'fan_only') ? (
                          <div className="space-y-5 pb-8">
                            ...
                            <div className="space-y-4">
                              ...
                              {(portalRole as any) === 'band' && (
                                <div className="space-y-1.5 mt-6 border-t border-zinc-900/60 pt-4">
                              ...
                              {/* Label Bio / Manifesto (Editable) - Moved to bottom */}
                              <div className="space-y-1 mt-4 border-t border-zinc-900/60 pt-4">
                                <div className="flex justify-between items-center">
                          ...
                          {/* Profile Blurb / Bio (At Bottom of Settings) */}
                      ...
                      {drawerCurrentView === 'payment' && (
                        <div className="space-y-4">
                          ...
                          <div className="pt-4 border-t border-zinc-900 space-y-3">
                            <div className="flex items-center justify-between">
                              <div>
                                <h4 className="text-[10px] font-black uppercase text-rose-500 tracking-wider font-mono">Link Digital Wallets (OAuth 2.0)</h4>
                                <p className="text-[9px] text-zinc-500 leading-normal">Authenticate securely with external digital wallet providers for 1-tap multi-card checkout.</p>
                              </div>
                              <span className="text-[8px] font-mono font-bold text-emerald-400 bg-emerald-950/50 border border-emerald-500/30 px-1.5 py-0.5 rounded">
                                Production Ready
                              </span>
                            </div>
                            ...
                            <div className="space-y-3">
                              ...
                              <div className="bg-zinc-950/80 border border-zinc-900 rounded-2xl p-3.5 space-y-3 transition-all hover:border-zinc-800">
                                <div className="flex items-center justify-between">
                                  <div className="flex items-center gap-3">
                                    ...
                                    <div>
                                      <div className="text-xs font-bold text-white flex items-center gap-1.5">
                                        ...
                                        {walletsState?.preferredWallet === 'google' && (
                                          <span className="text-[7.5px] font-mono text-amber-400 bg-amber-950/60 border border-amber-500/40 px-1 py-0.2 rounded font-bold">DEFAULT</span>
                                        )}
                                      </div>
                                      <div className="text-[9px] text-zinc-400 font-mono">
                                        {isGooglePayConnected ? `${walletsState.google.cards?.length || 1} Cards Linked • OAuth Live` : 'Disconnected (Requires OAuth)'}
                                      </div>
                                    </div>
                                  </div>
                                    ...
                                    {isGooglePayConnected ? (
                                      <div className="flex items-center gap-1.5">
                                        {walletsState?.preferredWallet !== 'google' && (
                                          <button
                                            type="button"
                                            onClick={() => handleSetPreferredWallet('google')}
                                            className="text-[8px] font-mono text-zinc-400 hover:text-amber-400 uppercase tracking-wider transition-colors px-1.5 py-0.5 rounded bg-zinc-900 border border-zinc-800"
                                        ...
                                        <button
                                          ...
                                          className="text-[8px] font-mono text-sky-400 hover:text-sky-300 uppercase tracking-wider transition-colors px-1.5 py-0.5 rounded bg-sky-950/50 border border-sky-500/30"
                                        >
                                          Manage
                                        </button>
                                        <button 
                                          type="button"
                                          onClick={() => handleDisconnectPayment('google')}
                                          className="text-[8px] font-bold text-zinc-500 hover:text-rose-400 transition-colors uppercase font-mono px-1.5 py-0.5 rounded bg-zinc-900 border border-zinc-800 cursor-pointer"
                                        >
                                          Unlink
                                        </button>
                                      </div>
                                      ...
                                      <button 
                                        type="button"
                                        onClick={() => handleConnectPayment('google')}
                                        className="px-3 py-1.5 bg-rose-600 hover:bg-rose-500 text-[9px] font-black uppercase tracking-wider text-white rounded-xl transition-all font-mono cursor-pointer shadow flex items-center gap-1"
                                      >
                                        <Lock className="w-2.5 h-2.5" /> Connect OAuth
                                      </button>
                                    )}
                                  </div>
                                </div>

                                {/* Connected Cards Selector */}
                                {isGooglePayConnected && walletsState.google.cards && walletsState.google.cards.length > 0 && (
                                  <div className="pt-2 border-t border-zinc-900 space-y-1.5">
                                    <div className="text-[8.5px] font-mono uppercase text-zinc-400 tracking-wider flex justify-between items-center">
                                      <span>Attached Cards (Choose Active):</span>
                                      <span className="text-emerald-400 font-bold text-[8px] flex items-center gap-1">
                                        <Check className="w-2.5 h-2.5" /> OAuth Verified
                                      </span>
                                    </div>
                                    <div className="grid grid-cols-1 gap-1.5">
                                      {walletsState.google.cards.map((card) => {
                                        ...
                                        return (
                                          <div
                                            key={card.id}
                                            onClick={() => handleSelectCard('google', card.id)}
                                            className={`p-2 rounded-xl border text-[10px] font-mono flex items-center justify-between cursor-pointer transition-all ${
                                              isSelected
                                ...
                                <div className="flex items-center justify-between">
                                  <div className="flex items-center gap-3">
                                    ...
                                    <div>
                                      <div className="text-xs font-bold text-white flex items-center gap-1.5">
                                        <span>Apple Pay</span>
                                        {walletsState?.preferredWallet === 'apple' && (
                                          <span className="text-[7.5px] font-mono text-amber-400 bg-amber-950/60 border border-amber-500/40 px-1 py-0.2 rounded font-bold">DEFAULT</span>
                                        )}
                                      </div>
                                      <div className="text-[9px] text-zinc-400 font-mono">
                                        {isApplePayConnected ? `${walletsState.apple.cards?.length || 1} Cards Linked • Touch ID Active` : 'Disconnected (Requires Passkey/OAuth)'}
                                      </div>
                                    </div>
                                  </div>
                                    ...
                                    {isApplePayConnected ? (
                                      <div className="flex items-center gap-1.5">
                                        {walletsState?.preferredWallet !== 'apple' && (
                                          <button
                                            type="button"
                                            onClick={() => handleSetPreferredWallet('apple')}
                                            className="text-[8px] font-mono text-zinc-400 hover:text-amber-400 uppercase tracking-wider transition-colors px-1.5 py-0.5 rounded bg-zinc-900 border border-zinc-800"
                                            title="Set as Default Wallet"
                                          >
                                            Set Default
                                        ...
                                        <button 
                                          type="button"
                                          onClick={() => handleDisconnectPayment('apple')}
                                          className="text-[8px] font-bold text-zinc-500 hover:text-rose-400 transition-colors uppercase font-mono px-1.5 py-0.5 rounded bg-zinc-900 border border-zinc-800 cursor-pointer"
                                        >
                                          Unlink
                                        </button>
                                      ...
                                      <button 
                                        type="button"
                                        onClick={() => handleConnectPayment('apple')}
                                        className="px-3 py-1.5 bg-zinc-900 hover:bg-zinc-800 border border-zinc-700 text-[9px] font-black uppercase tracking-wider text-white rounded-xl transition-all font-mono cursor-pointer shadow flex items-center gap-1"
                                      >
                                        <Fingerprint className="w-2.5 h-2.5 text-zinc-300" /> Connect Apple
                                      </button>
                                    )}
                                  </div>
                                </div>

                                {/* Connected Apple Cards */}
                                {isApplePayConnected && walletsState.apple.cards && walletsState.apple.cards.length > 0 && (
                                  <div className="pt-2 border-t border-zinc-900 space-y-1.5">
                                    <div className="text-[8.5px] font-mono uppercase text-zinc-400 tracking-wider flex justify-between items-center">
                                      <span>Apple Wallet Passes:</span>
                                      <span className="text-emerald-400 font-bold text-[8px] flex items-center gap-1">
                                        <Check className="w-2.5 h-2.5" /> Enclave Bound
                                      </span>
                                    </div>
                                    <div className="grid grid-cols-1 gap-1.5">
                                      {walletsState.apple.cards.map((card) => {
                                        ...
                                        return (
                                          <div
                                            key={card.id}
                                            onClick={() => handleSelectCard('apple', card.id)}
                                            className={`p-2 rounded-xl border text-[10px] font-mono flex items-center justify-between cursor-pointer transition-all ${
                                              isSelected
                                                ? 'bg-rose-950/30 border-rose-500 text-white font-bold'
                                                : 'bg-zinc-900/60 border-zinc-850 text-zinc-400 hover:border-zinc-700'
                                  ...
                                  <div className="flex items-center gap-3">
                                    ...
                                    <div>
                                      <div className="text-xs font-bold text-white flex items-center gap-1.5">
                                        ...
                                        {walletsState?.preferredWallet === 'paypal' && (
                                          <span className="text-[7.5px] font-mono text-amber-400 bg-amber-950/60 border border-amber-500/40 px-1 py-0.2 rounded font-bold">DEFAULT</span>
                                        )}
                                      </div>
                                      <div className="text-[9px] text-zinc-400 font-mono">
                                        {isPaypalConnected ? `${walletsState.paypal.cards?.length || 1} Methods Linked • OAuth Active` : 'Disconnected (Requires OAuth)'}
                                      </div>
                                    </div>
                                  </div>
                                    ...
                                    {isPaypalConnected ? (
                                      <div className="flex items-center gap-1.5">
                                        {walletsState?.preferredWallet !== 'paypal' && (
                                          <button
                                            type="button"
                                            onClick={() => handleSetPreferredWallet('paypal')}
                                            className="text-[8px] font-mono text-zinc-400 hover:text-amber-400 uppercase tracking-wider transition-colors px-1.5 py-0.5 rounded bg-zinc-900 border border-zinc-800"
                                            title="Set as Default Wallet"
                                          >
                                            Set Default
                                        ...
                                        <button 
                                          type="button"
                                          onClick={() => handleDisconnectPayment('paypal')}
                                          className="text-[8px] font-bold text-zinc-500 hover:text-rose-400 transition-colors uppercase font-mono px-1.5 py-0.5 rounded bg-zinc-900 border border-zinc-800 cursor-pointer"
                                        >
                                          Unlink
                                        </button>
                                      ...
                                      <button 
                                        type="button"
                                        onClick={() => handleConnectPayment('paypal')}
                                        className="px-3 py-1.5 bg-[#0079C1] hover:bg-[#005ea6] text-[9px] font-black uppercase tracking-wider text-white rounded-xl transition-all font-mono cursor-pointer shadow flex items-center gap-1"
                                      >
                                        <Lock className="w-2.5 h-2.5" /> Connect PayPal
                                      </button>
                                    )}
                                  </div>
                                </div>

                                {/* Connected PayPal Funding Sources */}
                                {isPaypalConnected && walletsState.paypal.cards && walletsState.paypal.cards.length > 0 && (
                                  <div className="pt-2 border-t border-zinc-900 space-y-1.5">
                                    <div className="text-[8.5px] font-mono uppercase text-zinc-400 tracking-wider flex justify-between items-center">
                                      <span>PayPal Funding Methods:</span>
                                      <span className="text-emerald-400 font-bold text-[8px] flex items-center gap-1">
                                        <Check className="w-2.5 h-2.5" /> Realtime Escrow
                                      </span>
                                    </div>
                                    <div className="grid grid-cols-1 gap-1.5">
                                      {walletsState.paypal.cards.map((card) => {
                                        ...
                                        return (
                                          <div
                                            key={card.id}
                                            onClick={() => handleSelectCard('paypal', card.id)}
                                            className={`p-2 rounded-xl border text-[10px] font-mono flex items-center justify-between cursor-pointer transition-all ${
                                              isSelected
                          ...
                          <div className="pt-4 border-t border-zinc-900">
                            <label 
                              onClick={handleToggleSingleTap}
                              className="flex items-center justify-between cursor-pointer group"
                            >
                              <div>
                                <div className="text-xs font-bold text-white group-hover:text-rose-400 transition-colors">Enable Single-Tap Buying</div>
                                <div className="text-[10px] text-zinc-500 leading-tight">Skip checkout details for all ticket presales & merchandise</div>
                              </div>
                              <div className={`w-10 h-5 rounded-full flex items-center p-0.5 transition-colors ${walletsState?.singleTapEnabled ? 'bg-rose-600' : 'bg-zinc-800'}`}>
                                <div className={`w-4 h-4 bg-white rounded-full shadow-sm transform transition-transform duration-200 ${walletsState?.singleTapEnabled ? 'translate-x-5' : 'translate-x-0'}`} />
                              </div>
                            </label>
                          </div>
                        </div>
              ...
              {/* Real OAuth Digital Wallet Authorization Modal */}
              <WalletOAuthModal
                isOpen={!!activeOAuthProvider}
                provider={activeOAuthProvider || 'google'}
                userProfile={props.userProfile}
                onClose={() => setActiveOAuthProvider(null)}
                onSuccess={(prov, card) => {
                  const refreshed = getStoredWallets(props.userProfile);
