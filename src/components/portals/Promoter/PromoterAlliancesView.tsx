...
interface PromoterAlliancesViewProps {
...
const DEFAULT_ALLIANCES = [
  {
    ...
    total_budget: 1200,
    primary_promoter_id: "promoter-lead-soren",
    partner_promoter_id: "promoter-typ-drusilla",
  ...
  },
  {
    ...
    total_budget: 1850,
    primary_promoter_id: "promoter-lead-soren",
    partner_promoter_id: "promoter-vec-vlad",
...
export default function PromoterAlliancesView({
  userProfile,
  triggerNotification,
  addLog
}: PromoterAlliancesViewProps) {
  const [alliances, setAlliances] = useState<any[]>(() => {
    try {
      const saved = localStorage.getItem('nexus_core_promoter_alliances');
  ...
  useEffect(() => {
    localStorage.setItem('nexus_core_promoter_alliances', JSON.stringify(alliances));
  }, [alliances]);
        ...
        if (supabase) {
          ...
          if (!error && data && data.length > 0) {
            const mapped = data.map((item: any) => ({
              ...
              primary_promoter_id: item.primary_promoter_id,
              partner_promoter_id: item.partner_promoter_id,
              partner_name: item.partner_promoter_id ? item.partner_promoter_id.slice(0, 5) : "Pending",
              partner_role: item.contract_specs?.partner_role || "Co-Producer",
              partners: item.contract_specs?.partners || [{
                name: item.partner_promoter_id ? item.partner_promoter_id.slice(0, 5) : "Pending",
  ...
  const handleCreateAlliance = async (e: React.FormEvent) => {
    ...
    const newAllianceItem = {
      ...
      total_budget: 0,
      primary_promoter_id: userProfile?.id || "promoter-lead-soren",
      partner_promoter_id: `partner-${Math.floor(Math.random() * 10000)}`,
                    ...
                    onKeyDown={(e) => {
                      if (e.key === 'Enter' && newMessage.trim()) {
                        ...
                        const msg = {
                          ...
                          sender: userProfile?.id || 'promoter-lead-soren',
                          senderName: "You",
